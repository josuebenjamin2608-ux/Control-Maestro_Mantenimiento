 ...
 * sendTechnicianAssignedNotification exclusivamente cuando REALMENTE crea
 * una fila nueva en MaintenanceRequestTechnician — nunca cuando el técnico
 * ya estaba activamente asignado, nunca si la operación falla — y que un
 * fallo de Telegram nunca afecte la asignación ya guardada. Se ejerce el
 * código real de assignTechnicianToRequest; solo se mockean el cliente de
 * Prisma (fake mínimo en memoria, sin tocar Postgres), next/cache y el
 * servicio de Telegram.
 */

const { fakeState, sendTelegramMock } = vi.hoisted(() => {
  return {
    fakeState: {
...
vi.mock("@/server/services/telegram.service", () => ({
  sendTechnicianAssignedNotification: sendTelegramMock,
}));
...
function seedTechnician(id: string, fullName: string, isActive = true) {
  fakeState.technicians.set(id, { id, isActive, fullName, employeeCode: `EMP-${id}` });
}
...
describe("assignTechnicianToRequest — disparo de notificación Telegram", () => {
  ...
  it("una nueva asignación llama a Telegram exactamente una vez", async () => {
  ...
  it("repetir la misma asignación no genera otra notificación", async () => {
  ...
  it("cambiar de técnico A a técnico B genera una notificación nueva para cada uno", async () => {
    await assignTechnicianToRequest(REQUEST_ID, TECH_A_ID);
    ...
    expect(sendTelegramMock).toHaveBeenNthCalledWith(1, expect.anything(), "Benjamin Arzuza");
    expect(sendTelegramMock).toHaveBeenNthCalledWith(2, expect.anything(), "Otro Técnico");
  });

  it("si falla el guardado de la asignación, nunca se llama a Telegram", async () => {
  ...
  it("un fallo de Telegram no revierte la asignación: la fila queda guardada de todas formas", async () => {
